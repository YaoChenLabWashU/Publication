function [epoch_response_intensity_norm_all,epoch_response_lft_norm_all,epoch_response_intensity_all,epoch_response_lft_all] = Epoch_based_calculation(all_epochs, drug_epochs, CyclePositions, baseline_acq_num, epoch_acq_num, EpochStartAcq_all, intensity_all, lft_all, FigureVisible)
...
for i_cycle = 1:length(CyclePositions)
    ...
    title('intensity')
    legend(cell_legends, 'AutoUpdate','off')
    hold on
...
%     ylim([3.8 4.15])
    ...
    saveas(gcf, ['cycle ', num2str(CyclePositions(i_cycle)),'.fig'])
    saveas(gcf, ['cycle ', num2str(CyclePositions(i_cycle)),'.png'])
...
save([stateYao.baseName, '_', currTime, CyclePositions_in_name, '.mat'], 'epoch_response_intensity_norm_all','epoch_response_lft_norm_all',...
