function [lft_all,intensity_all, EpochStartAcq_all, AcqTime_all] = FLIM_posthoc_dendriteMode_topPixels(CyclePositions)
...
for i=CyclePositions(1):CyclePositions(i_cycleposition)
    ...
    lft_baseline=[];
    num_roi=size(stateYao.Results.spc_calculateROIvals.Lifetime_TopPixels{i}{10}, 2);
    roi=1;
    for j=1:num_roi
        for k=1:nnz(stateYao.AcqTime(:,i))
            if isempty(stateYao.Results.spc_calculateROIvals.Lifetime_TopPixels{i}{k}) == 0
...
for i=CyclePositions(1):CyclePositions(i_cycleposition)
    ...
    for j=1:num_roi
        for k=1:1:nnz(stateYao.AcqTime(:,i))
            if isempty(stateYao.Results.spc_calculateROIvals.Projection_TopPixels{i}{k}) == 0
