...
success=0;
if length(state) && isfield(state,'laser') && isfield(state,'laser.laserControlOn')
...
if success
    ...
    if isnan(str2double(waveln))
        display(laserCOM);
        display('no laser is connected');
...
if isnan(str2double(shutter))
    display('no laser connected (no laserCOM)');
elseif str2double(shutter)
    shutter='open';
else
    shutter='CLOSED';
end
