...
stateYao.img_orig = spc.projects{1,1};
stateYao.ROI_orig = gui.gy.roiPositions{1};
...
stateYao.ROI{iImg} = gui.gy.roiPositions{1};
...
% for iImg = 7
    ...
    if stateYao.CyclePositions(iImg,stateYao.numCycle) ~= stateYao.numImage_orig;
        ...
        stateYao.imageStack(:,:,iImg) = spc.projects{1,1};
...
%         figure; imshow(imgBW);
        ...
        % Apply changes
        gui.gy.roiPositions{1} = stateYao.ROI{iImg};
        for roi = 1
        for type=1:2
