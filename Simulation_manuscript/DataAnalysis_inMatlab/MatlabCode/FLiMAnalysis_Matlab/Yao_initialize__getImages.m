    ...
    for iImg = size(stateYao.CyclePositions,1):-1:1
        if stateYao.CyclePositions(iImg,numCycle) ~= 0
            ...
            if isvalid==false
                ...
                if spc.datainfo.numberOfZSlices == 1 % AM 20230712
                    ...
                    %end
                elseif spc.datainfo.numberOfZSlices > 1 %for multi-slice acquisiions AM 20230712
                    for j = 1:spc.datainfo.numberOfZSlices % cycle through every slice
                        stateYao.images.origData.projects{numCycle,j}(:,:,iImg) =....
                          spc.projects{1,j};%Incorporated slice level data per Yao's edit AM 20230919
                        stateYao.images.origData.lifetimeMaps{numCycle,j}(:,:,iImg) =....
                            spc.lifetimeMaps{1,j};
                        stateYao.images.origData.rgbLifetimes{numCycle,j}(:,:,:,iImg) =....
                            spc.rgbLifetimes{1};
                        
                        % Apply filters
                        if ~isDendrite
                            % Nucleus
                            stateYao.images.filterDensity.rgbLifetimes{numCycle,j}(:,:,:,iImg) =...
                                Yao_applyFilter('density',1);
                            
                            stateYao.images.filterFFT.rgbLifetimes{numCycle,j}(:,:,:,iImg) =...
                                Yao_generic_applyFFTFilter( spc.rgbLifetimes{1,1} );
                        end
                    end
