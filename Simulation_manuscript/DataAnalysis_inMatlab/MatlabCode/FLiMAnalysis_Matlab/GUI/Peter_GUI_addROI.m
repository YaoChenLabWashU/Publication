function Peter_GUI_addROI(ROINucleusInput)
...
%     Yao_generic_loadImage(numImage)
    ...
    %Peter Replaced to allow for multiple ROI adding
%     iCell = 1;
% 
%     global gui
%     
...
%         size( stateYao.images.origData.rgbLifetimes{numCycle}(:,:,iImg) ,2) );
    ...
    iCell = size(gui.gy.roiPositions,2);

    for cell = 1:1:iCell
        stateYao.ROI{numCycle}{iImg}{cell} = gui.gy.roiPositions{1,cell};

        [stateYao.images.I_ROI_stack{numCycle}{iImg}(:,:,cell)] = Yao_generic_convertROI2I_ROI(...
            stateYao.ROI{numCycle}{iImg}{cell},...
            size( stateYao.images.origData.rgbLifetimes{numCycle}(:,:,iImg) ,1),...
            size( stateYao.images.origData.rgbLifetimes{numCycle}(:,:,iImg) ,2) );
    end
    
    % to-do: allow different cell IDs for different ROIs in dendritic mode?
    stateYao.cellIdx{numCycle}{iImg} = zeros(size(stateYao.images.I_ROI_stack{numCycle}{iImg},3),6);
    stateYao.cellIdx{numCycle}{iImg}(:, 1) = 1:size(stateYao.images.I_ROI_stack{numCycle}{iImg},3);
    stateYao.cellIdx{numCycle}{iImg}(:, 2) = 1:size(stateYao.images.I_ROI_stack{numCycle}{iImg},3); 
