...
def parse_input():
  ...
  parser.add_argument('--num_repeats', help = 'Number of repeats. Default=1', type=int,
                      default=1)
  parser.add_argument('--verbose_output', help = 'display verbose fitting output',
                      default=False, action='store_true')
  parser.add_argument('--calculate_lifetime', help = 'calculate empirical/fitted lifetime',
                      default=False, action='store_true')
...
def summarize_simulation(fitted_tau, fitting_results, empirical_lifetimes, histograms, output_path, verbose = False,
                         display_lifetime_information = False):
  """Summarize simulation repeats"""
  print('\n************** Simulation Sumary **************')
  if display_lifetime_information:
    simulation_summary = {'repeat': list(range(len(empirical_lifetimes))),
                          'empirical_lifetime': empirical_lifetimes,
                          'fitted_tau': fitted_tau}
    if len(empirical_lifetimes) > 0:
    ...
    if output_path:
      ...
      print(f'simulation summary saved to {os.path.join(output_path, "simulation_sumary.csv")}')
  ...
  if output_path:
    ...
    print(f'averaged histogram saved to {os.path.join(output_path, "averaged_histogram.csv")}')
...
def main():
  ...
  summarize_simulation(fitted_taus, fitting_results, empirical_lifetimes, histograms, args.output,
                       display_lifetime_information=args.calculate_lifetime)
