function FLIMsim256_noauto = FLIMsim256_noauto(SS, DC, AF, GeneratedPopulation)
global n;
...
%   replacement
load('DoubleExpPop256.mat'); % This file contains 92808 data points of a double exponential fit, with taus 2.6 and 0.9, and relative distribution of 0.5 and 0.5
...
load('prf_Yao_0529.mat'); % Load from measured prf in UREA_IRF_5 acqn 62, subtracted bg, divided by total number of photons.
SampleC=zeros(1, length(Sample));
for i=1:length(Sample)
    a=find(histc(rand(),[cumsum(prf(:))]))-50;
...
% load('autofluo_0722.mat'); %Load from a probability distribution of autofluorescence in Autofluorescence_2FLIM001, measured at 30um below the surface of an 
%   untransfected slice, at 920nm, zoom 10, 2.5mW power.
% Autofluorescence=zeros(1, AF); %Give each photon zero lifetime to start with.
% for i=1:AF
%     Autofluorescence(i)=find(histc(rand(),[cumsum(AutoFluoSim_0722(:))]))*(12.5/256); % Draw a lifetime for autofluorescence based on the probability specified in the measured autofluorescence file.
% end
