...
% LT added this for herself, but should be compatible with others as well
if lock_status == 0
    floats =  logical([1 0 1 0 1 1]);
end
...
global spc;
%spc.datainfo.pulseInt is defined during acqusition as 1E9/sync rate, but
%sync rate is pulled from spcm32 and sometimes incorrectly puts Inf
pulseI=spc.datainfo.pulseInt / spc.datainfo.psPerUnit*1000; %number of channels within a pulse interval
% disp(pulseI)
