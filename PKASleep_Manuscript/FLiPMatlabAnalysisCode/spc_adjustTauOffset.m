...
lifetime = lifetime(:);
[peak_y, peak_x] = max(lifetime);

t1_shift = t1(1:end-peak_x+1);
lifetime_shift = lifetime(peak_x:end);
shift = peak_x*spc.datainfo.psPerUnit/1000;
a = sum(lifetime_shift.*t1_shift)/sum(lifetime_shift)*spc.datainfo.psPerUnit/1000;

% uses avgTau OR avgTauTrunc (determined in spc.switches)
% gy 201111 should probably always use avgTauTrunc
spc.switchess{chan}.('EmpLife')=a;
% spc.switchess{chan}.('EmpLife')=a - shift; %YC: Different from Gary's version. offset calculated based on avgTau calculated to infinity so that the empirical lifetime is independent of fitting range.
spc.switchess{chan}.('Shift')=shift;
...
%LT commented out the next 3 lines because couldnt find field 122225
